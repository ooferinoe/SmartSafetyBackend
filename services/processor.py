...
def normalize_detections(resp):
    ...
    for d in raw:
        ...
        if "xmin" in d and "ymin" in d and "xmax" in d and "ymax" in d:
            ...
            "bbox" in d and isinstance(d["bbox"], (list, tuple)) and len(d["bbox"]) >= 4
        ):
...
def process_frame_from_model_response(
    model_resp: dict, background_tasks=None, dedupe_window_seconds: int = 30
):
    """
    ...
    logger.debug(
        "process_frame_from_model_response: compliance=%d, noncompliance=%d",
        len(compliance),
        len(violations),
    )
    return {
