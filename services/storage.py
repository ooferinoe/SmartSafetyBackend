...
def _normalize_doc(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ats = (
        out.get("alertSentTo")
        or out.get("alert_sent_to")
        or out.get("alert_email")
        or out.get("alertEmail")
    )
    if ats is None:
        out["alertSentTo"] = []
...
def find_recent_similar(
    camera_id: str, violation_type: str, within_seconds: int = 30
) -> Optional[str]:
    """
    ...
    q = (
        violations_ref.where(filter=FieldFilter("camera_id", "==", camera_id))
...
def add_violation(doc: dict, dedupe_window_seconds: int = 30):
    ...
    if cam and vtype and dedupe_window_seconds > 0:
        existing_id = find_recent_similar(
            cam, vtype, within_seconds=dedupe_window_seconds
        )
        if existing_id:
    ...
    normalized["_ts"] = (
        ...
        else firestore.firestore.SERVER_TIMESTAMP
    )
...
def increment_daily_scans(count: int):
    ...
    stats_ref.set(
        {"total_scans": firestore.Increment(count), "date": today_str}, merge=True
    )
