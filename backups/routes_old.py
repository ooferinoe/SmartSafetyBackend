...
def filter_overlaps(detections):
    ...
    for i, det in enumerate(detections):
        ...
        for j, other in enumerate(detections):
            ...
            if iou > 0.5 and (
                (
                    cls_name == "Safety Glasses"
                    and other["name"] == "Improper Safety Glasses"
                )
                or (
                    cls_name == "Improper Safety Glasses"
                    and other["name"] == "Safety Glasses"
                )
            ):
...
def send_email_alert_from_backend(violation_data, footage_url):
    ...
    to_email = (
        (violation_data.get("alertSentTo") or [])[0]
        if violation_data.get("alertSentTo")
        else None
    )
    if not to_email:
        return
...
def final_upload_and_update(temp_video_path, violation_docs):
    ...
    try:
        ...
        upload_result = cloudinary.uploader.upload(
            temp_video_path, resource_type="video", folder="violations"
        )
        public_id = upload_result.get("public_id")
        footage_url = f"https://res.cloudinary.com/{cloud_name}/video/upload/f_mp4/{public_id}.mp4"
        if footage_url:
            print(
                f"INFO (Thread): MP4 URL generated. Updating docs and sending email..."
            )
...
def process_upload_and_alert(violation_docs):
    ...
    try:
        ...
        upload_thread = threading.Thread(
            target=final_upload_and_update, args=(temp_video_path, violation_docs)
        )
        upload_thread.start()
    except Exception as e:
...
def detect_ipcam(background_tasks: BackgroundTasks):
    ...
    if is_on_cooldown or not detection_lock.acquire(blocking=False):
        return JSONResponse(
            content={"message": "System is on cooldown or busy."}, status_code=429
        )
    try:
        cap = cv2.VideoCapture(STREAM_URL)
        if not cap.isOpened():
            return JSONResponse(
                content={"error": "Failed to open stream"}, status_code=500
            )
        ret, frame = cap.read()
        cap.release()
        if not ret:
            return JSONResponse(
                content={"error": "Failed to get frame"}, status_code=500
            )
        results = model(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        detections = filter_overlaps(results.pandas().xyxy[0].to_dict(orient="records"))
        unresolved_detections = [
            d for d in detections if d["name"] in UNRESOLVED_CLASSES
        ]
        ...
        is_on_cooldown = True
        print(
            f"INFO: Detected {len(unresolved_detections)} violation(s). Locking system and starting background task."
        )
        ...
        for det in unresolved_detections:
            violation_data = {
                ...
                "date": ph_time.strftime("%m/%d/%Y"),
                "violationType": det["name"],
                "status": "Unresolved",
                "confidence": f"{det['confidence'] * 100:.0f}",
                "footageId": "CAM 001",
                "alertSentTo": ["danieljosesagun@gmail.com"],
                "alertSent": False,
                "footageUrl": None,
            }
        ...
        background_tasks.add_task(
            process_upload_and_alert, violation_docs=violation_doc_ids
        )
...
def get_frame_detections():
    ...
    if not cap.isOpened():
        return JSONResponse(
            content={"error": "Stream is busy or unavailable"}, status_code=503
        )
    ret, frame = cap.read()
    cap.release()
    if not ret:
        return JSONResponse(content={"error": "Failed to get frame"}, status_code=500)
    results = model(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
    detections = filter_overlaps(results.pandas().xyxy[0].to_dict(orient="records"))
    response = [
        {
    ...
    return JSONResponse(
        content={"detections": response, "width": width, "height": height}
    )
