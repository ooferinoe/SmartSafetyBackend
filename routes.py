...
def final_upload_and_update(temp_video_path, violation_docs):
    try:
        if temp_video_path:
            ...
            if footage_url:
                print(
                    f"INFO (Thread): {footage_url} generated. Updating violation docs..."
                )
        ...
        if (
            temp_video_path
            and isinstance(temp_video_path, str)
...
def detect_ipcam(background_tasks: BackgroundTasks):
    ...
    try:
        result = process_frame_from_model_response(
            current_detection, background_tasks=background_tasks
        )
        ...
        for v in violations:
            logger.info(
                f"Unresolved violation: {v.get('violationId')} ({v.get('violationType')})"
        ...
        violation_ids = [
            v.get("violationId") for v in violations if v.get("violationId")
        ]
        ...
        response_payload["unresolved"] = [
            v.get("violationType") or v.get("type") for v in violations
        ]
        return JSONResponse(response_payload)
    #     return JSONResponse({
...
async def upload_video(background_tasks: BackgroundTasks, violation_ids: list):
    ...
    if output_frame is None:
        return JSONResponse(
            {"error": "No frames available from webcam."}, status_code=400
        )
    ...
    if not out.isOpened():
        return JSONResponse(
            {"error": "Failed to initialize video writer."}, status_code=500
        )
